...
function intentShadowPanelContent(shadow) {
  const stats = shadow || {};
  const value = (key) => Number.isFinite(Number(stats[key])) ? Number(stats[key]) : '—';
  const outcome = escapeHtml(String(stats.lastOutcome || '暂无结果'));
  const mismatchFields = Object.entries(stats.mismatchByField || {})
    .filter(([, count]) => Number(count) > 0)
    .sort((a, b) => Number(b[1]) - Number(a[1]))
    .slice(0, 5)
    .map(([field, count]) => `<span class="intent-shadow-tag">${escapeHtml(field)} · ${Number(count)}</span>`)
    .join('');
  const latestSample = Array.isArray(stats.samples) && stats.samples.length ? stats.samples[0] : null;
  const sampleCopy = latestSample
    ? `最近样本：${escapeHtml((latestSample.differences || []).join('、') || '无字段差异')}`
    : '暂未记录结构化差异样本';
  return `
    <div class="intent-shadow-copy">
      <div>
        <span class="config-kicker">LLM INTENT OBSERVABILITY</span>
        <strong>${stats.enabled === true ? '旁路对比已开启' : '旁路对比当前关闭'}</strong>
        <p>新 Tool Calling 只与旧意图结果做对比，不参与用户响应，也不会直接修改行程。</p>
      </div>
      <button class="secondary mini-btn" data-action="probe-admin-intent-capability">探测统一规划协议</button>
    </div>
    <div class="intent-shadow-grid">
      <div><span>已提交</span><b>${value('submitted')}</b></div>
      <div><span>已完成</span><b>${value('completed')}</b></div>
      <div><span>匹配</span><b>${value('matched')}</b></div>
      <div><span>不匹配</span><b>${value('mismatched')}</b></div>
      <div><span>语义匹配</span><b>${value('semanticMatched')}</b></div>
      <div><span>语义不匹配</span><b>${value('semanticMismatched')}</b></div>
      <div><span>失败/拒绝</span><b>${value('failed')} / ${value('rejected')}</b></div>
      <div><span>最近结果</span><b>${outcome}</b></div>
    </div>
    <div class="intent-shadow-diagnostics">
      <span>高频差异</span>
      <div class="intent-shadow-tags">${mismatchFields || '<em>暂无</em>'}</div>
      <small>${sampleCopy}</small>
    </div>
    <div class="intent-shadow-footnote">统计范围：当前 Java 进程生命周期 · API Key 状态：${stats.configured === true ? '已配置' : '未配置'}</div>
  `;
}
...
export function getFilteredAttractions(items) {
  const q = (state.adminAttractionQuery || '').trim().toLowerCase();
  const cat = (state.adminAttractionCategory || '').trim();
  return (items || []).filter((item) => {
    if (cat === '室内' && !item.indoor) return false;
    if (cat === '户外' && item.indoor) return false;
    if (cat && cat !== '室内' && cat !== '户外' && item.category !== cat && !(item.tags || []).includes(cat)) return false;
    if (q) {
      const haystack = [item.name, item.id, item.district, item.category, item.walkDifficulty, ...(item.tags || [])].join(' ').toLowerCase();
...
export function renderAdminAttractionItemsHtml(items, docsByEntity = new Map()) {
  if (!items || !items.length) {
    return '<div class="notice" style="grid-column: 1 / -1; padding: 24px; text-align: center;">没有找到符合筛选条件的景点项目。</div>';
  }
  return items.map((item) => {
    ...
    const isDining = item.isDining || item.category === '美食';
    return `
      <div class="admin-attraction-row" data-attraction-id="${escapeHtml(item.id || '')}">
        <div class="attraction-main-info">
          <span class="attraction-icon ${item.indoor ? 'is-indoor' : ''} ${isDining ? 'is-dining' : ''}">${isDining ? '美食' : (item.indoor ? '室内' : '户外')}</span>
          <div>
...
export function updateAdminAttractionListInDOM() {
  const container = document.getElementById('admin-attraction-list-container');
  if (!container) return;
  const overview = state.adminOverview || {};
  const attractions = overview.attractions || {};
  const items = Array.isArray(attractions.items) ? attractions.items : [];
  ...
  const filtered = getFilteredAttractions(items);
  container.innerHTML = renderAdminAttractionItemsHtml(filtered, docsByEntity);
  const countBadge = document.getElementById('admin-attraction-count-badge');
  if (countBadge) countBadge.textContent = `${filtered.length} / ${items.length} 个项目`;
  const chips = document.querySelectorAll('#admin-attraction-filter-bar .chip');
  chips.forEach((c) => {
    const cat = c.dataset.category || '';
    c.classList.toggle('selected', (state.adminAttractionCategory || '') === cat);
  });
}

export function getFilteredUsers(users) {
  const q = (state.adminUserQuery || '').trim().toLowerCase();
  const role = (state.adminUserRoleFilter || '').trim();
  return (users || []).filter((u) => {
    if (role && u.role !== role && (role === 'traveler' ? u.role === 'admin' : u.role !== 'admin')) return false;
    if (q) {
      const match = [u.name, u.email, u.id].join(' ').toLowerCase();
...
export function renderAdminUserRowsHtml(users) {
  if (!users || !users.length) {
    return '<div class="notice" style="padding: 24px; text-align: center;">当前没有匹配的账号。</div>';
  }
  return users.map((u) => {
...
export function updateAdminUserListInDOM() {
  const mount = document.querySelector('[data-admin-user-list]');
  if (!mount) return;
  const users = Array.isArray(state.adminOverview?.users) ? state.adminOverview.users : [];
  const filtered = getFilteredUsers(users);
  mount.innerHTML = renderAdminUserRowsHtml(filtered);
  const chips = document.querySelectorAll('#admin-user-role-filter-bar .chip');
  chips.forEach((c) => {
    const r = c.dataset.role || '';
    c.classList.toggle('selected', (state.adminUserRoleFilter || '') === r);
  });
}
...
export function adminView() {
  ...
  const intentShadow = state.adminIntentShadowStats || null;
  const allDocs = Array.isArray(state.adminDocs) ? state.adminDocs : [];
  const attractions = overview.attractions || {};
  ...
  const filteredAttractions = getFilteredAttractions(attractionItems);
      ...
      <div class="admin-workspace-layout">
        <aside class="admin-sidebar">
          ...
          <nav class="admin-control-nav">
            ${adminSections.map(([key, label, desc, icon]) => {
              let badgeText = '';
              if (key === 'attractions' && attractionAvailable) badgeText = `${attractions.total}`;
              else if (key === 'users') badgeText = `${users.length}`;
              else if (key === 'feedback' && feedbackTotal) badgeText = `${feedbackTotal}`;
              else if (key === 'knowledge' && corpus.documents) badgeText = `${corpus.documents}`;
              return `
                <button class="admin-control-link ${adminSection === key ? 'is-active' : ''}" type="button" data-action="admin-section" data-section="${key}">
                  <span>${icon}</span>
                  <div class="admin-nav-info">
                    <strong>${label}</strong>
                    <small>${desc}</small>
                  </div>
                  ${badgeText ? `<span class="admin-nav-badge">${badgeText}</span>` : ''}
                  <b>进入 →</b>
                </button>
              `;
            }).join('')}
          ...
          <div class="admin-sidebar-status">
            <div class="admin-status-indicator">
              <span class="status-dot ${analytics.available ? 'online' : 'standby'}"></span>
              <small>${analytics.available ? 'Java 服务实时在线' : '等待服务连接'}</small>
            </div>
          </div>
        </aside>
        ...
        <div class="admin-content-area">
            ...
            ${attractionAvailable ? `
              ...
              <div class="admin-assets-layout">
                ...
                <div class="panel panel-pad asset-coverage-panel">
                  ...
                  <p class="muted">目录展示每个景点的结构化资料、适配标签和已关联知识条目；支持按室内/户外、分类与关键词实时过滤。</p>
              ...
              <div class="panel panel-pad admin-attraction-catalog">
                <div class="admin-attraction-catalog-head">
                  <div>
                    <div class="panel-title">景点与文旅资产目录</div>
                    <p>实时检索与分类核对已接入文旅资产及结构化知识。</p>
                  </div>
                  <span id="admin-attraction-count-badge">${filteredAttractions.length} / ${attractionItems.length} 个项目</span>
                </div>
                <div class="admin-attraction-toolbar">
                  <input id="admin-attraction-search" value="${escapeHtml(state.adminAttractionQuery || '')}" placeholder="搜索景点名称、区域（渝中区/南岸区）、分类或标签…" />
                  <div class="chip-row" id="admin-attraction-filter-bar">
                    ${['全部', '室内', '户外', '文创', '自然', '人文', '夜景', '美食'].map((cat) => `
                      <button class="chip ${(state.adminAttractionCategory === cat || (!state.adminAttractionCategory && cat === '全部')) ? 'selected' : ''}" data-action="admin-attraction-filter" data-category="${cat === '全部' ? '' : cat}">${cat}</button>
                    `).join('')}
                  </div>
                </div>
                <div class="admin-attraction-list admin-attraction-list-full" id="admin-attraction-list-container">
                  ${renderAdminAttractionItemsHtml(filteredAttractions, docsByEntity)}
                </div>
              </div>
          ...
          <section class="admin-stream-section admin-section-panel ${adminSection === 'rerank' ? 'is-current' : 'is-hidden'}" data-admin-section="rerank">
            ...
            <div class="panel panel-pad intent-shadow-panel" id="admin-intent-shadow-panel-content">${intentShadowPanelContent(intentShadow)}</div>
          </section>
            ...
            <div class="panel panel-pad admin-users-panel">
              <div class="admin-user-directory-intro">
                ...
                <div class="admin-user-overview" aria-label="账号概览">
                  ...
                  <span><b data-admin-admin-count>${adminCount}</b> 位管理员</span>
                  <span><b>${travelerCount}</b> 位旅行者</span>
                </div>
              </div>
              <div class="admin-user-toolbar">
                <div class="chip-row" id="admin-user-role-filter-bar">
                  ${[
                    { role: '', label: `全部 (${users.length})` },
                    { role: 'admin', label: `系统管理员 (${adminCount})` },
                    { role: 'traveler', label: `普通旅行者 (${travelerCount})` }
                  ].map((opt) => `
                    <button class="chip ${(state.adminUserRoleFilter === opt.role || (!state.adminUserRoleFilter && !opt.role)) ? 'selected' : ''}" data-action="admin-user-role-filter" data-role="${opt.role}">${opt.label}</button>
                  `).join('')}
                </div>
                <input id="admin-user-search" value="${escapeHtml(state.adminUserQuery || '')}" placeholder="按姓名、邮箱或ID搜索账号…" />
              </div>
              <div class="admin-user-list" data-admin-user-list id="admin-user-list-mount">
                ${renderAdminUserRowsHtml(filteredUsers)}
              </div>
            </div>
          </section>
